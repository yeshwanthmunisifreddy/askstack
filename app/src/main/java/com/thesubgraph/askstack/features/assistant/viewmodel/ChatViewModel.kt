...
import com.thesubgraph.askstack.features.assistant.data.local.storage.SecurePreferences
...
import com.thesubgraph.askstack.features.assistant.domain.usecase.CreateConversationUseCase
import com.thesubgraph.askstack.features.assistant.domain.usecase.GetMessagesUseCase
import com.thesubgraph.askstack.features.assistant.domain.usecase.SendMessageUseCase
...
class ChatViewModel @Inject constructor(
    ...
    fun initializeChat(
        ...
        if (conversationId != null) {
            ...
            createNewConversation(assistantId, initialMessage)
        } else {
    ...
    private fun createNewConversation(assistantId: String, initialMessage: String? = null) {
        viewModelScope.launch {
            _uiState.value = _uiState.value.copy(isLoading = true, error = null)
            createConversationUseCase(assistantId, "New Chat").flowOn(Dispatchers.IO)
                .collect { result ->
                    when (result) {
                        ...
                        is ValueResult.Success -> {
                            ...
                            if (!initialMessage.isNullOrBlank()) {
                                _messageInput.value = initialMessage
                                sendMessage()
                            }
                        }
                    }
    ...
    fun stopStreaming() {
        viewModelScope.launch {
            ...
            streamingMessage?.let { message ->
                try {
                    sendMessageUseCase.stopStreaming(message.id)
                } catch (e: Exception) {
                }
            }
        }
